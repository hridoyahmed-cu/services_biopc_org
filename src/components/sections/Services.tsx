            ...
            <Reveal key={service.id} delay={(i % 3) * 90}>
              <article className="panel panel-hover group flex h-full flex-col p-7">
                <div className="flex items-baseline justify-between gap-4">
                  <span className="font-mono text-[0.7rem] tracking-[0.2em] text-[#38bdf8]">
                    {service.number}
                  </span>
                  <span className="h-px flex-1 bg-[var(--hairline)] transition-colors group-hover:bg-[rgba(56,189,248,0.4)]" />
                </div>

                <h3 className="mt-5 font-display text-[1.4rem] leading-tight text-white">
                  {service.title}
                </h3>
                ...
                <ul className="mt-6 space-y-2.5 border-t border-[var(--hairline)] pt-6">
                  {service.items.map((item) => (
                    <li key={item} className="flex items-start gap-2.5">
                      <span className="mt-0.5 h-4 w-4 shrink-0 text-[#4ade9a]">
                        <CheckIcon />
                      </span>
                      <span className="text-[0.875rem] leading-snug text-[#cddff5]">
                        {item}
                      </span>
                    </li>
                  ))}
