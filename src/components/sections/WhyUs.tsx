      ...
      <div className="shell">
            ...
            return (
              <Reveal key={item.title} delay={i * 90}>
                <article className="panel panel-hover h-full p-6">
                  <span className="grid h-11 w-11 place-items-center rounded-xl border border-[rgba(56,189,248,0.25)] bg-[rgba(56,189,248,0.08)] p-2.5 text-[#5cc6fb]">
                    <Icon />
                  </span>
        ...
        <Reveal delay={120}>
          <dl className="mt-6 grid gap-px overflow-hidden rounded-2xl border border-[var(--hairline)] bg-[var(--hairline)] sm:grid-cols-2 lg:grid-cols-4">
            {METRICS.map((m) => (
              <div key={m.label} className="bg-[#071228] px-6 py-7">
                <dt className="font-display text-[2.1rem] leading-none text-white">
                  {m.value}
                </dt>
                <dd className="mt-2.5 text-[0.875rem] font-medium text-[#c3d8f4]">
                  {m.label}
                </dd>
